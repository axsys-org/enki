...
static er_val boot_bytes_nat(boot_ctx* ctx, const char* bytes_c,
                             size_t bytes_s) {
  ...
  uint64_t* limb_q =
      ctx->loc_a->alloc(ctx->loc_a->ctx, limb_s * sizeof(uint64_t));
  if (limb_q == NULL) {
    return 0;
  }
...
static wisp_env_entry* boot_env_clone(boot_ctx* ctx, wisp_env_entry* env) {
  ...
  for (wisp_env_entry* ent = env; ent != NULL; ent = ent->next) {
    wisp_env_entry* copy =
        ctx->loc_a->alloc(ctx->loc_a->ctx, sizeof(wisp_env_entry));
    if (copy == NULL) {
      return NULL;
    }
    *copy = (wisp_env_entry){
...
static bool boot_env_put(boot_ctx* ctx, wisp_env_entry** env,
                         wisp_env_entry* src) {
...
static wisp_env_entry* boot_env_merge(boot_ctx* ctx, wisp_env_entry* old,
                                      wisp_env_entry* new_env) {
...
static er_val boot_make_row(boot_ctx* ctx, int argc, char** argv) {
  ...
  er_val* arg_v =
      ctx->loc_a->alloc(ctx->loc_a->ctx, (size_t)argc * sizeof(er_val));
  if (arg_v == NULL) {
    return 0;
  }
