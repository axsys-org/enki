...
/* Called before either a direct host effect or a coordination request begins.
 * Embedders use this notification for scheduling policy; it does not replace
 * or service the effect. */
typedef void (*pl_effect_observer)(void* ctx, pl_thread* t);
void pl_thread_set_effect_observer(pl_thread* t, pl_effect_observer observer,
                                   void* ctx);
