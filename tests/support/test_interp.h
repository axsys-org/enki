...
{
    if(enki_test_store_path_s[0] == '\0') return;
...
static void enki_test_make_store(void)
{
    for(size_t k = 0; k < 1000; k++) {
        int n = snprintf(
            ...
            sizeof(enki_test_store_path_s),
            "/tmp/enki-test-%lu-%lu",
            (unsigned long)getpid(),
            enki_test_store_seq_s++);
        if(n < 0 || (size_t)n >= sizeof(enki_test_store_path_s)) abort();
        if(mkdir(enki_test_store_path_s, 0700) == 0) return;
