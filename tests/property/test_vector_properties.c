...
static void* accounting_alloc(void* ctx, size_t size)
{
    ...
    account->live_bytes += size;
    return header + 1;
}

static void* accounting_realloc(void* ctx, void* ptr, size_t size)
{
    if (ptr == NULL) {
        return accounting_alloc(ctx, size);
    }

    accounting_allocator* account = ctx;
    allocation_header* old_header = ((allocation_header*)ptr) - 1;
    size_t old_size = old_header->size;
    allocation_header* new_header = realloc(old_header, sizeof(*new_header) + size);
    if (new_header == NULL) {
        return NULL;
    }

    new_header->size = size;
    account->live_bytes = (account->live_bytes - old_size) + size;
    return new_header + 1;
}
...
static void accounting_free(void* ctx, void* ptr)
{
    ...
    account->live_allocations -= 1;
    account->live_bytes -= header->size;
    free(header);
}
...
static void* generate_sequence(theft_rng* rng, void* ctx)
{
    ...
    generated->len = theft_rng_range(rng, MAX_SEQUENCE_LEN + 1u);
...
static bool property_push_increases_len(const void* generated, void* ctx, char* out, size_t out_len)
{
    ...
    for (size_t i = 0; i < sequence->len; i += 1) {
...
static bool property_get_pushed_item(const void* generated, void* ctx, char* out, size_t out_len)
{
    ...
    size_t index = enki_vector_len(vector);
    void* value = (void*)sequence->values[0];
    bool ok = enki_vector_push(vector, value) == ENKI_OK && enki_vector_get(vector, index) == value;

    enki_vector_destroy(vector);
    if (!ok || account.live_allocations != 0 || account.live_bytes != 0) {
        message(out, out_len, "pushed item was not retrievable at old length or leaked");
        return false;
    }
                                            ...
                                            size_t out_len)
{
    ...
    for (size_t i = sequence->len; i > 0; i -= 1) {
                                                    ...
                                                    size_t out_len)
{
    ...
    enki_vector_destroy(vector);
    if (!ok || account.live_allocations != 0 || account.live_bytes != 0) {
        message(out, out_len, "reserve changed length, decreased capacity, or leaked");
        return false;
    }
...
static int run_one(const char* name, theft_check_fn check, uint64_t seed)
{
...
int main(void)
{
    ...
    failures += run_one("len(push(v, x)) == len(v) + 1", property_push_increases_len,
                        seed ^ UINT64_C(0x101));
    failures +=
        run_one("get(push(v, x), len(v)) == x", property_get_pushed_item, seed ^ UINT64_C(0x202));
    failures += run_one("push sequence then equal pops returns empty",
                        property_pushes_then_pops_empty, seed ^ UINT64_C(0x303));
    failures += run_one("reserve never decreases capacity or changes len",
