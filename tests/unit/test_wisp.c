...
static void assert_strnat_bytes(er_val value_v, const char* expected_c) {
  ...
  size_t expected_limbs_s =
      (expected_s + sizeof(uint64_t) - 1u) / sizeof(uint64_t);
...
static void assert_eval_fails(char* input_c, const char* expected_prefix_c) {
  ...
  rt->err_f = 0;
  cr_assert_not_null(rt->msg_c);
  cr_assert(strncmp(rt->msg_c, expected_prefix_c, strlen(expected_prefix_c)) ==
                0,
...
Test(wisp, parse_nested_form) {
  char* rest_c = NULL;
  er_app* outer =
      assert_row(parse_input("((#pin \"B\") (\"Up\" i v r))", &rest_c), 2);
...
Test(wisp, law_body_hash_juxtaposition_splices_raw_expression) {
  ...
  assert_small_strnat(eval_input("(#bind id (#pin (#law \"id\" (id x) x)))"),
                      "id");
...
Test(wisp, numeric_pin66_wrapper_compiles_pessimistically) {
  ...
  er_law* law =
      assert_law(eval_input("(#law \"caller\" (caller ignored) (add 20 22))"));
  const er_op* code = er_law_label_code_const(law, 0);
  cr_assert_not_null(code);
  ...
  for (size_t k = 0; k < law->bc_v[0].op_s; k++) {
    ...
    case OP_RET:
      break;
    default:
      cr_assert_fail("compiler emitted opcode %u at op %zu",
                     (unsigned int)code[k].tag, k);
    }
  ...
  cr_assert_eq(eval_input("((#law \"caller\" (caller ignored) (add 20 22)) 0)"),
               42);
}
...
Test(wisp, macro_export_and_system_shadowing_match_plan_assembler) {
  assert_small_strnat(
      eval_input("(#macro zap (#law \"zap\" (zap env form) 0))"), "zap");
...
Test(wisp, user_macro_results_are_forced_to_nf_before_syntax_interpretation) {
  assert_strnat_bytes(
      eval_input("(#macro make-law (#law \"make-law\" (make-law env form) "
                 "sig((0 \"made\" \"x\")) "
                 "(0 \"#law\" (1 \"made\") sig \"x\")))"),
      "make-law");
...
Test(wisp, app_evaluates_raw_operands_in_order) {
  assert_strnat_bytes(
      eval_input("(#bind choose (#law \"choose\" (choose x y) x))"), "choose");
...
Test(wisp, law_macroexpands_all_binds_and_body_before_compiling_binds) {
  assert_strnat_bytes(
      eval_input(
          "(#macro install (#law \"install\" (install env form) "
          "(0 \"#macro\" \"late\" "
...
Test(wisp, law_self_reference_is_usable_as_value) {
  assert_strnat_bytes(eval_input("(#bind self (#law \"self\" (self x) self))"),
                      "self");
...
Test(wisp, long_nat_symbols_are_environment_and_local_keys) {
  ...
  assert_strnat_bytes(
      eval_input(
          "(#bind abcdefgh (#law \"abcdefgh\" (abcdefgh longargx) longargx))"),
      "abcdefgh");
...
Test(wisp, recursive_program_validates_extended_snippet) {
  ...
  assert_small_strnat(
      eval_input("(#bind dechelp (#law \"dechelp\" (dechelp count x) "
                 "(if (eq (inc count) x) count "
                 "(dechelp (inc count) x))))"),
      "dechelp");
  cr_assert_gt(assert_law(eval_input("dechelp"))->bc_s, 0);
  assert_small_strnat(
      eval_input("(#bind dec (#law \"dec\" (dec x) (dechelp 0 x)))"), "dec");
