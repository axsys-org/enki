...
static bool bytecode_contains_wide(uint8_t* bc_b, size_t bc_len_s, uint8_t op_b,
                                   uint16_t operand) {
...
Test(compiler, emits_push_const_wide_for_const_indices_above_u8) {
  ...
  cr_assert(bytecode_contains_wide((uint8_t*)enki_vector_data(bc_b),
                                   enki_vector_len(bc_b), OP_PUSH_CONST_WIDE,
                                   256));
...
Test(compiler, emits_pick_wide_for_stack_indices_above_u8) {
  ...
  cr_assert(bytecode_contains_wide((uint8_t*)enki_vector_data(bc_b),
                                   enki_vector_len(bc_b), OP_PICK_WIDE, 299));
...
Test(compiler, emits_apply_wide_for_generic_apps_above_u8_arity) {
  ...
  cr_assert(bytecode_contains_wide((uint8_t*)enki_vector_data(bc_b),
                                   enki_vector_len(bc_b), OP_APPLY_WIDE, 256));
