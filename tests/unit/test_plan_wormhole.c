...
TEST_FIXTURE(wormhole, wormhole_init, NULL)
...
TEST(wormhole, live_and_dead_collection) {
  ...
  ASSERT(pl_is_wormhole(rt.t->vstack[0]));
  ASSERT_EQ(pl_wormhole_token(rt.t->vstack[0]), UINT64_C(0xfeedface));
  ASSERT_EQ(host_state.releases, 0);

  rt.t->vsp = 0;
  pl_gc_collect_now(rt.t);
  ASSERT_EQ(host_state.releases, 1);
  ASSERT_EQ(host_state.last_token, UINT64_C(0xfeedface));
  test_rt_free(&rt);
  ASSERT_EQ(host_state.releases, 1);
}
...
TEST(wormhole, close_is_idempotent) {
  ...
  ASSERT(pl_wormhole_is_closed(wormhole));
  pl_wormhole_close(wormhole);
  ASSERT_EQ(host_state.releases, 1);
...
TEST(wormhole, clone_retains_per_wrapper) {
  ...
  pl_vpush(rt.t, clone);
  ASSERT_EQ(host_state.retains, 1);
  ASSERT_EQ(pl_wormhole_token(rt.t->vstack[1]), 73);
  ...
  rt.t->vsp = 0;
  pl_gc_collect_now(rt.t);
  ASSERT_EQ(host_state.releases, 2);
  test_rt_free(&rt);
}
...
TEST(wormhole, heap_growth_and_teardown_release_once) {
  ...
  ASSERT_EQ(host_state.releases, 0);
  ASSERT_EQ(pl_wormhole_token(t->vstack[0]), 99);
...
TEST(wormhole, blackhole_operations_fail_and_debug_is_opaque) {
  ...
  ASSERT_STR_EQ(shown, "<wormhole>");
  ASSERT_NULL(strstr(shown, "123456789abcdef0"));
