...
static void run_law(uint8_t* bc_b, size_t bc_len_s, enki_value* consts_v,
                    size_t n_const_s) {
  enki_value law = enki_law_alloc(fixture_interp->gc, 0, 0, 0, bc_len_s,
                                  n_const_s, bc_b, consts_v);
...
static void run_law_with_args(uint8_t* bc_b, size_t bc_len_s,
                              enki_value* consts_v, size_t n_const_s,
                              size_t arity_s, enki_value* args_v) {
  enki_value law = enki_law_alloc(fixture_interp->gc, arity_s, 0, 0, bc_len_s,
                                  n_const_s, bc_b, consts_v);
...
Test(interp_bytecode, runs_multiple_op66_ops_in_one_law) {
  uint8_t bc_b[] = {
      OP_PUSH_CONST, 0, OP_PUSH_CONST, 1,        OP_OP66,   OP66_ADD,
      OP_PUSH_CONST, 2, OP_OP66,       OP66_MUL, OP_RETURN,
  };
...
Test(interp_bytecode, dup_and_pop_leave_expected_stack) {
  uint8_t bc_b[] = {
      OP_PUSH_CONST, 0,       OP_DUP,   OP_PUSH_CONST, 1,
      OP_POP,        OP_OP66, OP66_ADD, OP_RETURN,
  };
...
Test(interp_bytecode, op_apply_calls_law_from_bytecode) {
  ...
  enki_value inc = enki_law_alloc(fixture_interp->gc, 1, 0, 0, sizeof(inc_bc),
                                  0, inc_bc, NULL);
  uint8_t bc_b[] = {
      OP_PUSH_CONST, 0, OP_PUSH_CONST, 1, OP_APPLY, 1, OP_RETURN,
  };
...
Test(interp_bytecode, apply_wide_applies_more_than_255_args) {
  ...
  enki_value id = enki_law_alloc(fixture_interp->gc, 256, 0, 0, sizeof(id_bc),
                                 0, id_bc, NULL);
  enki_value consts_v[257];
  consts_v[0] = id;
