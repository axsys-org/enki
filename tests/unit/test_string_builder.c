...
static enki_allocator* make_test_allocator(test_allocator_state* state,
                                           int with_realloc) {
...
static void assert_build_bytes(enki_string_builder* sb, const char* expected,
                               size_t expected_len) {
  size_t got_len = 777;
...
Test(enki_string_builder,
     growth_falls_back_to_alloc_copy_free_without_realloc) {
  test_allocator_state state;
  enki_allocator* allocator;
  enki_string_builder sb;
  char expected[21];
...
Test(enki_string_builder,
     allocation_failure_during_growth_marks_builder_failed) {
  test_allocator_state state;
  enki_string_builder sb;
  size_t i;
  size_t len = 999;
...
Test(enki_string_builder,
     reset_clears_content_and_recovers_from_allocator_failure) {
  test_allocator_state state;
  enki_string_builder sb;
  size_t i;
...
Test(enki_string_builder,
     build_allocation_failure_returns_null_without_failing_builder) {
  test_allocator_state state;
  enki_string_builder sb;
  size_t len = 999;
  char* out;
...
Test(enki_string_builder,
     build_with_allocator_uses_requested_output_allocator) {
