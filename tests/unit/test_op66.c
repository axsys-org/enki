...
Test(op66, structural_eq_compares_apps_laws_and_pins) {
  ...
  enki_value law_a = enki_law_alloc(fixture_interp->gc, 2, 11, 22, sizeof(bc_b),
                                    0, bc_b, NULL);
  enki_value law_b = enki_law_alloc(fixture_interp->gc, 2, 11, 22, sizeof(bc_b),
                                    0, bc_b, NULL);
  enki_value law_c = enki_law_alloc(fixture_interp->gc, 1, 11, 22, sizeof(bc_b),
                                    0, bc_b, NULL);
  ...
  enki_value pin_a =
      enki_pin_alloc(fixture_interp->gc, hash_b, 42, 1, subpins_a);
  enki_value pin_b =
      enki_pin_alloc(fixture_interp->gc, hash_b, 42, 1, subpins_b);
...
Test(op66, equal_forces_before_structural_compare) {
  ...
  enki_value inc =
      enki_law_alloc(fixture_interp->gc, 1, 0, 0, sizeof(bc_b), 0, bc_b, NULL);
...
Test(op66, bytecode_dispatch_runs_op66_add) {
  uint8_t bc_b[] = {
      OP_PUSH_CONST, 0, OP_PUSH_CONST, 1, OP_OP66, OP66_ADD, OP_RETURN,
  };
  enki_value consts_v[] = {2, 3};
  enki_value law = enki_law_alloc(fixture_interp->gc, 0, 0, 0, sizeof(bc_b), 2,
                                  bc_b, consts_v);
