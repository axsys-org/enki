...
static enki_value eval_op66_arity(enki_value input_v) {
  ...
  cr_assert_eq(enki_plan_apply(&fixture_plan, pin_b, 1, &row_v, &out_v),
               ENKI_ERROR_OK);
  return out_v;
}
...
Test(plan, op66_arity_reports_only_raw_laws) {
  enki_value raw_law_v =
      enki_law_alloc(fixture_interp->gc, 3, 0, 0, 0, 0, NULL, NULL);
...
Test(plan, treewalks_law_body_with_vars_and_application) {
  ...
  enki_value law_v =
      enki_law_alloc(fixture_interp->gc, 2, 0, body_v, 0, 0, NULL, NULL);
  enki_value args_v[] = {20, 22};
  enki_value out_v = 0;

  cr_assert_eq(enki_plan_apply(&fixture_plan, law_v, 2, args_v, &out_v),
               ENKI_ERROR_OK);
...
Test(plan, partial_app_spines_flatten_before_reduce) {
  ...
  cr_assert_eq(enki_plan_apply(&fixture_plan, partial_v, 1, &arg_v, &out_v),
               ENKI_ERROR_OK);
...
Test(plan, op0_pin_pins_whnf_value) {
  ...
  cr_assert_eq(enki_plan_apply(&fixture_plan, pin_zero_v, 1, &row_v, &out_v),
               ENKI_ERROR_OK);
...
Test(plan, law_body_zero_resolves_to_self) {
  enki_value law_v =
      enki_law_alloc(fixture_interp->gc, 1, 0, 0, 0, 0, NULL, NULL);
  enki_value arg_v = 99;
  enki_value out_v = 0;

  cr_assert_eq(enki_plan_apply(&fixture_plan, law_v, 1, &arg_v, &out_v),
               ENKI_ERROR_OK);
