...
static pl_val jplan_effect(void* ctx, void* scope, pl_thread* t, pl_host_op op,
  ...
  (void)scope;
  jplan_state* s = ctx;
  ASSERT_EQ(op, PL_HOST_OP_JPLAN_EVAL);
  ASSERT_LT(s->effects, 4);
  size_t call = s->effects++;

  ASSERT(pl_is_wormhole(t->vstack[ab]));
  ASSERT_FALSE(pl_wormhole_is_closed(t->vstack[ab]));
  ...
  if (t->vstack[ab + 1] != 0) {
    ...
    if (pl_app_n(row) != 0) {
      ASSERT(pl_is_wormhole(pl_app_args(row)[0]));
...
TEST_FIXTURE(jplan, jplan_init, NULL)
...
TEST(jplan, lazy_handles_and_rows_are_opaque_and_interceptor_is_bypassed) {
  ...
  ASSERT_EQ(status, PL_RUN_DONE, "status=%d error=%s", (int)status,
            t->exn_msg == NULL ? "<none>" : t->exn_msg);
  ASSERT(pl_is_wormhole(pl_thread_result(t)));
  ASSERT_EQ(pl_wormhole_token(pl_thread_result(t)), 100);
  ASSERT_EQ(state.effects, 1);
  ASSERT_EQ(state.interceptor_calls, 0);
  ASSERT_EQ(state.environments[0], 11);
  ASSERT_EQ(state.object_counts[0], 1);
  ASSERT_EQ(state.objects[0], 21);
  ASSERT_EQ(state.sources[0], 7);
  ASSERT_NEQ(state.rows[0], original_row);
  ASSERT_FALSE(pl_is_wormhole(pl_app_args(pl_ptr(original_row))[0]));
  test_rt_free(&rt);
}
...
TEST(jplan, empty_rows_and_nested_eval_results_work) {
  ...
  ASSERT_EQ(jplan_run(t), PL_RUN_DONE);
  ASSERT_EQ(state.effects, 2);
  ASSERT_EQ(state.object_counts[0], 0);
  ASSERT_EQ(state.sources[0], 1);
  ASSERT_EQ(state.object_counts[1], 1);
  ASSERT_EQ(state.objects[1], 100);
  ASSERT_EQ(state.sources[1], 2);
  ASSERT_EQ(pl_wormhole_token(pl_thread_result(t)), 101);
  test_rt_free(&rt);
}
...
TEST(jplan, ordinary_strict_continuations_still_force_results) {
  ...
  if (setjmp(c.jb) == 0) {
    ...
    FAIL_TEST("expected the JPLAN result to retain blackhole semantics");
  }
  pl_catch_unwind(t, &c);
