...
static enki_value make_law(size_t arity_s, uint8_t* bc_b, size_t bc_len_s) {
  return enki_law_alloc(fixture_interp->gc, arity_s, 0, 0, bc_len_s, 0, bc_b,
                        NULL);
}

static enki_value make_law_with_consts(size_t arity_s, uint8_t* bc_b,
                                       size_t bc_len_s, enki_value* consts_v,
                                       size_t n_const_s) {
  return enki_law_alloc(fixture_interp->gc, arity_s, 0, 0, bc_len_s, n_const_s,
                        bc_b, consts_v);
}

static enki_value compile_body(enki_value body_v) {
  enki_vector* bc_b =
      enki_vector_create_sized(enki_allocator_system(), sizeof(uint8_t));
  enki_vector* consts_v =
      enki_vector_create_sized(enki_allocator_system(), sizeof(enki_value));
  ...
  enki_value law = enki_law_alloc(
      fixture_interp->gc, 0, 0, body_v, enki_vector_len(bc_b),
      enki_vector_len(consts_v), (uint8_t*)enki_vector_data(bc_b),
...
Test(runtime_stats, small_nat_inc_stays_on_immediate_fast_path) {
  ...
  cr_log_info("small inc stats: tmp_alloc=%" PRIu64 " tmp_bytes=%" PRIu64
              " heap_alloc=%" PRIu64 " heap_bytes=%" PRIu64
              " immediate_results=%" PRIu64 " big_results=%" PRIu64,
...
Test(runtime_stats, big_nat_fallback_still_records_scratch_and_heap_churn) {
  ...
  cr_log_info("big inc stats: tmp_alloc=%" PRIu64 " tmp_bytes=%" PRIu64
              " heap_alloc=%" PRIu64 " heap_bytes=%" PRIu64
              " immediate_results=%" PRIu64 " big_results=%" PRIu64,
              fixture_interp->stats.nat_tmp_alloc_s,
              fixture_interp->stats.nat_tmp_bytes_s,
...
Test(runtime_stats, bytecode_law_inc_records_apply_eval_and_op_counts) {
  uint8_t bc_b[] = {
      OP_PICK, 0, OP_OP66, OP66_INC, OP_RETURN,
  };
  ...
  cr_log_info(
      ...
      fixture_interp->stats.interp_step_s, fixture_interp->stats.law_enter_s,
      fixture_interp->stats.apply_s, fixture_interp->stats.apply_exact_s,
...
Test(runtime_stats, app_apply_shape_surface_reports_under_exact_and_over) {
  ...
  cr_log_info(
      ...
      fixture_interp->stats.apply_s, fixture_interp->stats.apply_row_s,
      fixture_interp->stats.apply_exact_s, fixture_interp->stats.apply_under_s,
      fixture_interp->stats.apply_over_s, fixture_interp->stats.law_enter_s,
      fixture_interp->stats.interp_step_s, fixture_interp->stats.whnf_s,
...
Test(runtime_stats, op66_immediate_nat_surface_stays_off_nat_allocators) {
  ...
  enki_value consts_v[C_COUNT] = {
      [C_ZERO] = 0,        [C_ONE] = 1,     [C_TWO] = 2,       [C_THREE] = 3,
      [C_FOUR] = 4,        [C_FIVE] = 5,    [C_SEVEN] = 7,     [C_EIGHT] = 8,
      [C_NINE] = 9,        [C_TEN] = 10,    [C_FIFTEEN] = 15,  [C_SIXTEEN] = 16,
      [C_THIRTY_TWO] = 32, [C_BYTE] = 0xAB, [C_WORD] = 0x1234, [C_DONE] = 42,
  };
...
#undef EMIT
  ...
  cr_log_info("op66 immediate surface stats: steps=%" PRIu64 " nat_tmp=%" PRIu64
              " nat_heap=%" PRIu64 " add=%" PRIu64 " mul=%" PRIu64
              " bits=%" PRIu64 " store8=%" PRIu64,
...
Test(runtime_stats, row_structural_ops_survive_nat_heap_churn) {
  ...
  cr_log_info("row structural nat heap churn stats: nat_tmp=%" PRIu64
              " nat_heap=%" PRIu64 " nat_heap_bytes=%" PRIu64
              " gc_collect=%" PRIu64 " gc_copy=%" PRIu64 " gc_live=%" PRIu64
              " gc_high=%" PRIu64 " final_sp=%zu",
              fixture_interp->stats.nat_tmp_alloc_s,
              fixture_interp->stats.nat_heap_alloc_s,
              fixture_interp->stats.nat_heap_bytes_s,
              ...
              fixture_interp->stats.gc_high_water_bytes_s, fixture_interp->sp);
...
Test(runtime_stats, eval_whnf_ind_cache_prevents_second_thunk_apply) {
  ...
  uint64_t second_step_delta_s =
      fixture_interp->stats.interp_step_s - first_steps_s;
  uint64_t second_thunk_delta_s =
      fixture_interp->stats.whnf_app_thunk_s - first_thunk_s;
  ...
  fprintf(stderr,
          ...
          first_whnf_s, first_apply_s, first_steps_s, second_whnf_delta_s,
          second_apply_delta_s, second_step_delta_s, second_thunk_delta_s);
...
Test(runtime_stats, eval_nf_pin_law_app_surface_forces_hashes_and_caches) {
  ...
  enki_value pin_v =
      enki_pin_alloc(fixture_interp->gc, hash_b, inner_v, 1, subpins_v);
  ...
  cr_log_info(
      ...
      fixture_interp->stats.whnf_s, fixture_interp->stats.whnf_app_thunk_s,
      fixture_interp->stats.apply_s, fixture_interp->stats.apply_exact_s,
      fixture_interp->stats.interp_step_s, fixture_interp->stats.law_enter_s,
      fixture_interp->stats.nat_tmp_alloc_s,
      fixture_interp->stats.nat_heap_alloc_s);
...
Test(runtime_stats, eval_bytecode_factorial_uses_eval_whnf_without_exploding) {
  ...
  uint8_t else_bc[] = {
      ...
      OP_PICK,       0,          // n
      OP_OP66,       OP66_MUL,   OP_RETURN,
  };
  enki_value else_consts[] = {0, dec};
  enki_value else_law =
      make_law_with_consts(1, else_bc, sizeof(else_bc), else_consts, 2);

  uint8_t fac_bc[] = {
      OP_PICK,       0,       OP_PUSH_CONST, 0, // 0
      OP_OP66,       OP66_EQ, OP_PUSH_CONST, 1, // 1
      OP_PUSH_CONST, 2,                         // id
      OP_PUSH_CONST, 3,                         // else law
      OP_PICK,       0,                         // n
      OP_APPLY,      2,                         // thunk: id else n
      OP_OP66,       OP66_IF, OP_RETURN,
  };
  enki_value fac_consts[] = {0, 1, id, else_law};
  enki_value fac =
      make_law_with_consts(1, fac_bc, sizeof(fac_bc), fac_consts, 4);
  ...
  run_until_base_frame();
  enki_value result_v =
      enki_eval_whnf(fixture_interp, fixture_interp->stack_v[0]);
  uint64_t dex_q = wisp_now_ns();

  fprintf(
      stderr,
      "eval fac 8 stats: time %" PRIu64 " ns, result=%" PRIu64 " whnf=%" PRIu64
      " whnf_thunk=%" PRIu64 " apply=%" PRIu64 " exact=%" PRIu64
      " under=%" PRIu64 " over=%" PRIu64 " steps=%" PRIu64 " law_enter=%" PRIu64
      " op66_force=%" PRIu64 " op66_mul=%" PRIu64 " nat_tmp=%" PRIu64
      " nat_heap=%" PRIu64,
      (dex_q - sin_q), result_v, fixture_interp->stats.whnf_s,
      fixture_interp->stats.whnf_app_thunk_s, fixture_interp->stats.apply_s,
      fixture_interp->stats.apply_exact_s, fixture_interp->stats.apply_under_s,
      fixture_interp->stats.apply_over_s, fixture_interp->stats.interp_step_s,
...
Test(runtime_stats, full_surface_one_shot_reports_runtime_oom_perf) {
  ...
  enki_value inner_v = enki_app_alloc(fixture_interp->gc,
                                      fixture_interp->stack_v[pin_root_s], 1);
  ...
  uint8_t hash_b[32] = {0};
  enki_value pin_v =
      enki_pin_alloc(fixture_interp->gc, hash_b,
                     fixture_interp->stack_v[pin_root_s + 1], 0, NULL);
  ...
  cr_log_info(
      ...
      fixture_interp->stats.interp_step_s, fixture_interp->stats.law_enter_s,
      fixture_interp->stats.apply_s, fixture_interp->stats.apply_exact_s,
      fixture_interp->stats.apply_under_s, fixture_interp->stats.apply_over_s,
      fixture_interp->stats.whnf_s, fixture_interp->stats.whnf_app_thunk_s,
      fixture_interp->stats.op66_s[OP66_INC],
      fixture_interp->stats.op66_s[OP66_ADD],
      fixture_interp->stats.nat_tmp_alloc_s,
      fixture_interp->stats.nat_tmp_bytes_s,
      fixture_interp->stats.nat_heap_alloc_s,
      fixture_interp->stats.nat_heap_bytes_s, fixture_interp->stats.gc_alloc_s,
      fixture_interp->stats.gc_locked_alloc_s,
      fixture_interp->stats.gc_collect_s, fixture_interp->stats.gc_copy_s,
      fixture_interp->stats.gc_live_bytes_s,
      fixture_interp->stats.gc_high_water_bytes_s, fixture_interp->sp);
