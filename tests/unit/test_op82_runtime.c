...
static void reset_fd_table(void) {
  ...
  enki_global_fd_table.slots[0] =
      (enki_fd_slot){.live = true, .fd = 0, .refs = 1};
  enki_global_fd_table.slots[1] =
      (enki_fd_slot){.live = true, .fd = 1, .refs = 1};
  enki_global_fd_table.slots[2] =
      (enki_fd_slot){.live = true, .fd = 2, .refs = 1};
}
...
Test(op82_runtime, op82_write_and_read_use_shared_fd_table) {
  ...
  fixture_interp->stack_v[1] =
      enki_bytes_to_nat(fixture_interp, bytes, sizeof(bytes));
  fixture_interp->sp = 2;
  ...
  size_t len = 0;
  uint8_t* out =
      enki_nat_to_bytes(fixture_interp, fixture_interp->stack_v[0], &len);
...
Test(op82_runtime, op82_output_and_warn_write_to_standard_streams) {
  ...
  fixture_interp->stack_v[0] =
      enki_bytes_to_nat(fixture_interp, out_bytes, sizeof(out_bytes));
  fixture_interp->sp = 1;
  ...
  fixture_interp->stack_v[0] =
      enki_bytes_to_nat(fixture_interp, err_bytes, sizeof(err_bytes));
  fixture_interp->sp = 1;
...
Test(op82_runtime, op82_read_file_and_stamp_return_file_contents_and_mtime) {
  ...
  fixture_interp->stack_v[0] =
      enki_bytes_to_nat(fixture_interp, (uint8_t*)path, strlen(path));
  fixture_interp->sp = 1;
  ...
  fixture_interp->stack_v[0] =
      enki_bytes_to_nat(fixture_interp, missing, sizeof(missing) - 1);
  fixture_interp->sp = 1;
  op82_read_file(fixture_interp);
  cr_assert_eq(fixture_interp->stack_v[0], 0);
...
Test(op82_runtime, send_caps_rejects_bad_caps_rows_and_bad_cap_handles) {
  ...
  enki_value null_caps =
      enki_alloc_row(fixture_interp->gc, 0, 1, null_cap_args);
  fixture_interp->stack_v[0] = 1;
  fixture_interp->stack_v[1] = 88;
