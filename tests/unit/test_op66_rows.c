...
Test(op66_rows, hd_last_init_and_ix_read_app_shape)
{
    ...
    enki_app* init = (enki_app*)ENKI_TO_PTR(fixture_interp->stack_v[0]);
...
Test(op66_rows, slice_weld_and_up_allocate_expected_rows)
{
    ...
    fixture_interp->sp = 3;
    cr_assert_eq(((enki_app*)ENKI_TO_PTR(fixture_interp->stack_v[2]))->args_v[2], 30);
    op66_up(fixture_interp);

    enki_app* updated_v = (enki_app*)ENKI_TO_PTR(fixture_interp->stack_v[0]);
