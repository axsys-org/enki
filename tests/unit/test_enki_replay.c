...
TEST(replay, schedulers_intercept_effects_independently) {
  ...
  ASSERT_EQ(er_scheduler_run(right), ER_RUN_IDLE);
  ASSERT_EQ(er_scheduler_run(left), ER_RUN_IDLE);
  ASSERT_EQ(er_log_events(left_log), 1);
  ASSERT_EQ(er_log_events(right_log), 1);
  ...
  ASSERT_EQ(er_scheduler_run(left), ER_RUN_IDLE);
  ASSERT_EQ(er_scheduler_run(right), ER_RUN_IDLE);
  ASSERT_EQ(er_actor_result(left_actor), left_now);
  ASSERT_EQ(er_actor_result(right_actor), right_now);
  ASSERT_EQ(er_scheduler_log_cursor(left), 1);
  ASSERT_EQ(er_scheduler_log_cursor(right), 1);
