...
static enki_bst_tree make_tree(const item* xs, size_t n) {
  ...
  for (i = 0; i < n; ++i) {
    enki_bst_node* inserted =
        enki_bst_tree_put(&t, xs[i].key, xs[i].value, xs[i].macro);
...
static enki_bst_tree make_balanced7(void) {
  static const item xs[] = {
      {8, 80, 0}, {4, 40, 1},   {12, 120, 0}, {2, 20, 0},
      {6, 60, 1}, {10, 100, 1}, {14, 140, 0},
  };
...
static void collect_unique_nodes(enki_bst_node* root, enki_bst_node** nodes,
...
static intptr_t pack_subtree_size(enki_bst_key key, int macro, intptr_t value,
                                  intptr_t left, intptr_t right, void* user) {
...
Test(enki_bst_persistent, put_into_empty_tree_returns_singleton) {
  enki_bst_node* root =
      enki_bst_put_persistent(test_allocator(), NULL, 42, V(420), 1);
...
Test(enki_bst_persistent,
     insert_copies_search_path_and_preserves_original_tree) {
