...
uint8_t* enki_nat_to_bytes(enki_interpreter* i, enki_value a, size_t* len) {
  *len = (size_t)enki_nat_bytes(i->gc, a);
  ...
  if (!res)
    enki_interp_throw(i, ENKI_ERROR_OOM, a);
...
enki_value enki_nat_alloc_big(enki_gc* gc, size_t n_limbs_s,
                              mp_limb_t limbs[]) {
...
enki_value enki_nat_alloc(enki_gc* gc, mp_limb_t* out, size_t n_limbs_s) {
  ...
  if (n == 0) {
    if (gc != NULL && gc->root != NULL)
      gc->root->stats.nat_immediate_result_s++;
    return (enki_value)0;
  }
...
static int enki_nat_cmp_v(const enki_nat_view* a_nv,
                          const enki_nat_view* b_nv) {
...
enki_value enki_nat_le(enki_value a_v, enki_value b_v) {
  if (!IS_PTR(a_v) && !IS_PTR(b_v))
    return a_v <= b_v ? 1 : 0;
  return (enki_nat_lt(a_v, b_v) || (enki_nat_eq(a_v, b_v))) ? (enki_value)1
                                                            : (enki_value)0;
}
...
enki_value enki_nat_ge(enki_value a_v, enki_value b_v) {
  if (!IS_PTR(a_v) && !IS_PTR(b_v))
    return a_v >= b_v ? 1 : 0;
  return (enki_nat_gt(a_v, b_v) || (enki_nat_eq(a_v, b_v))) ? (enki_value)1
                                                            : (enki_value)0;
}
enki_value enki_nat_inc(enki_gc* gc, enki_value a_v) {
  ENKI_PROFILE_ZONE("enki_nat_inc");
  if (!IS_PTR(a_v) && a_v < ENKI_SMALL_MAX)
    return a_v + 1;
  enki_nat_view a_nv;
  view_of_nat(a_v, &a_nv);
  if (a_nv.n_limbs_s == 0)
    return (enki_value)1;
  mp_limb_t* out =
      enki_nat_tmp_alloc(gc, sizeof(mp_limb_t) * (a_nv.n_limbs_s + 1));
...
enki_value enki_nat_add(enki_gc* gc, enki_value a_v, enki_value b_v) {
  ...
  if (!IS_PTR(a_v) && !IS_PTR(b_v) && a_v <= ENKI_SMALL_MAX - b_v)
    return a_v + b_v;
  enki_nat_view a_nv;
  enki_nat_view b_nv;
  view_of_nat(a_v, &a_nv);
  ...
  mp_limb_t carry_q = mpn_add(out, big_nv->limbs, big_nv->n_limbs_s,
                              small_nv->limbs, small_nv->n_limbs_s);
  out[n_s - 1] = carry_q;
...
enki_value enki_nat_sub(enki_gc* gc, enki_value a_v, enki_value b_v) {
  ...
  view_of_nat(b_v, &b_nv);
  if (a_nv.n_limbs_s == 0)
    return (enki_value)0; // no negatives
  if (b_nv.n_limbs_s == 0)
    return a_v;
  if (enki_nat_cmp_v(&a_nv, &b_nv) < 0)
    return (enki_value)0; // a_v < b_v
  mp_limb_t* out = enki_nat_tmp_alloc(gc, a_nv.n_limbs_s * sizeof(mp_limb_t));
...
enki_value enki_nat_mul(enki_gc* gc, enki_value a_v, enki_value b_v) {
  ...
  if (!IS_PTR(a_v) && !IS_PTR(b_v) && (a_v == 0 || b_v <= ENKI_SMALL_MAX / a_v))
    return a_v * b_v;
  enki_nat_view a_nv;
  enki_nat_view b_nv;
  view_of_nat(a_v, &a_nv);
  view_of_nat(b_v, &b_nv);
  if (a_nv.n_limbs_s == 0 || b_nv.n_limbs_s == 0)
    return (enki_value)0;
...
enki_value enki_nat_div(enki_gc* gc, enki_value a_v, enki_value b_v) {
  ...
  if (!IS_PTR(a_v) && !IS_PTR(b_v) && b_v != 0)
    return a_v / b_v;
  enki_nat_view a_nv;
  enki_nat_view b_nv;
  view_of_nat(a_v, &a_nv);
...
enki_value enki_nat_lsh(enki_gc* gc, enki_value a_v, enki_value shift_v) {
  ...
  if (!IS_PTR(a_v) && !IS_PTR(shift_v) && shift_v < 63 &&
      a_v <= (ENKI_SMALL_MAX >> shift_v))
    return a_v << shift_v;
  enki_nat_view a_nv;
  view_of_nat(a_v, &a_nv);
  if (IS_PTR(shift_v))
    enki_interp_throw(gc->root, ENKI_ERROR_TYPE, shift_v);
  if (shift_v == 0)
    return a_v;
  ...
  mp_limb_t carry_q = mpn_lshift(out + word_shift_s, a_nv.limbs, a_nv.n_limbs_s,
                                 (unsigned int)bit_shift_s);
  out[n_s - 1] = carry_q;
  return enki_nat_alloc(gc, out, n_s);
}
...
enki_value enki_nat_rsh(enki_gc* gc, enki_value a_v, enki_value shift_v) {
  ...
  if (!IS_PTR(a_v) && !IS_PTR(shift_v))
    return shift_v >= 63 ? 0 : a_v >> shift_v;
  enki_nat_view a_nv;
  view_of_nat(a_v, &a_nv);
  if (IS_PTR(shift_v))
    enki_interp_throw(gc->root, ENKI_ERROR_TYPE, shift_v);
  ...
  size_t word_shift_s = (size_t)shift_v / 64;
  size_t bit_shift_s = (size_t)shift_v % 64;
  if (word_shift_s >= a_nv.n_limbs_s)
    return (enki_value)0;
...
enki_value enki_nat_bex(enki_gc* gc, enki_value bit_v) {
  if (!IS_PTR(bit_v) && bit_v < 63)
    return ((enki_value)1 << bit_v);
  if (IS_PTR(bit_v))
    enki_interp_throw(gc->root, ENKI_ERROR_TYPE, bit_v);
...
enki_value enki_nat_test(enki_gc* gc, enki_value bit_v, enki_value a_v) {
  if (!IS_PTR(bit_v) && !IS_PTR(a_v))
    return bit_v < 63 ? ((a_v >> bit_v) & 1) : 0;
  enki_nat_view a_nv;
  view_of_nat(a_v, &a_nv);
  if (IS_PTR(bit_v))
    enki_interp_throw(gc->root, ENKI_ERROR_TYPE, bit_v);
...
enki_value enki_nat_set(enki_gc* gc, enki_value bit_v, enki_value a_v) {
  if (!IS_PTR(bit_v) && !IS_PTR(a_v) && bit_v < 63)
    return a_v | ((enki_value)1 << bit_v);
  enki_nat_view a_nv;
  view_of_nat(a_v, &a_nv);
  if (IS_PTR(bit_v))
    enki_interp_throw(gc->root, ENKI_ERROR_TYPE, bit_v);
...
enki_value enki_nat_bits(enki_gc* gc, enki_value a_v) {
  (void)gc;
  if (!IS_PTR(a_v))
    return a_v == 0 ? 0 : (enki_value)(64 - __builtin_clzll(a_v));
  enki_nat_view a_nv;
  view_of_nat(a_v, &a_nv);
  if (a_nv.n_limbs_s == 0)
    return (enki_value)0;
  ...
  size_t top_s =
      top_limb_q == 0 ? 0 : (64u - (size_t)__builtin_clzl(top_limb_q));
  return (enki_value)(top_s + ((a_nv.n_limbs_s - 1) * 64));
}
enki_value enki_nat_bytes(enki_gc* gc, enki_value a_v) {
  if (!IS_PTR(a_v))
    return a_v == 0 ? 0 : (enki_value)((64 - __builtin_clzll(a_v) + 7) / 8);
  return (enki_value)((enki_nat_bits(gc, a_v) + 7) / 8);
}
enki_value enki_nat_trunc(enki_gc* gc, enki_value width_v, enki_value a_v) {
  if (!IS_PTR(width_v) && !IS_PTR(a_v))
    return width_v >= 63 ? a_v : (a_v & (((enki_value)1 << width_v) - 1));
  if (IS_PTR(width_v))
    enki_interp_throw(gc->root, ENKI_ERROR_TYPE, width_v);
  enki_nat_view a_nv;
  view_of_nat(a_v, &a_nv);
  if (a_nv.n_limbs_s == 0)
    return (enki_value)0;
...
enki_value enki_nat_load8(enki_gc* gc, enki_value index_i, enki_value a_v) {
  if (!IS_PTR(index_i) && !IS_PTR(a_v))
    return index_i < 8 ? ((a_v >> (index_i * 8)) & 0xFF) : 0;
  if (IS_PTR(index_i))
    enki_interp_throw(gc->root, ENKI_ERROR_TYPE, index_i);
  enki_nat_view a_nv;
  view_of_nat(a_v, &a_nv);
  ...
  if (a_nv.n_limbs_s <= limb_index_i)
    return (enki_value)0;
...
enki_value enki_nat_store8(enki_gc* gc, enki_value index_i, enki_value byte_b,
  ...
  if (!IS_PTR(index_i) && !IS_PTR(byte_b) && !IS_PTR(a_v) && index_i < 8) {
    ...
    enki_value res_v =
        (a_v & ~((enki_value)0xFF << shift_v)) | ((byte_b & 0xFF) << shift_v);
    if (res_v <= ENKI_SMALL_MAX)
      return res_v;
  }
  if (IS_PTR(index_i))
    enki_interp_throw(gc->root, ENKI_ERROR_TYPE, index_i);
  if (IS_PTR(byte_b))
    enki_interp_throw(gc->root, ENKI_ERROR_TYPE, byte_b);
  enki_nat_view a_nv;
  view_of_nat(a_v, &a_nv);
  size_t limb_index_i = (size_t)index_i / sizeof(mp_limb_t);
  size_t byte_offset_o = (size_t)index_i % sizeof(mp_limb_t);
  size_t n_s =
      a_nv.n_limbs_s <= limb_index_i ? limb_index_i + 1 : a_nv.n_limbs_s;
  mp_limb_t* out = enki_nat_tmp_alloc(gc, n_s * sizeof(mp_limb_t));
  memset(out, 0, n_s * sizeof(mp_limb_t));
  memcpy(out, a_nv.limbs, a_nv.n_limbs_s * sizeof(mp_limb_t));
...
enki_value enki_nat_nib(enki_gc* gc, enki_value index_i, enki_value a_v) {
  if (!IS_PTR(index_i) && !IS_PTR(a_v))
    return index_i < 16 ? ((a_v >> (index_i * 4)) & 0xF) : 0;
  if (IS_PTR(index_i))
    enki_interp_throw(gc->root, ENKI_ERROR_TYPE, index_i);
  enki_nat_view a_nv;
  view_of_nat(a_v, &a_nv);
  ...
  if (limb_index_i >= a_nv.n_limbs_s)
    return (enki_value)0;
