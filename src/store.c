...
static enki_error er_store_buf_push(er_store_buf* buf, const void* data,
                                    size_t len_s) {
  if (len_s == 0) {
    return ENKI_ERROR_OK;
  }
...
static enki_error er_store_buf_push_u64(er_store_buf* buf, uint64_t val_q) {
  for (size_t k = 0; k < 8u; k++) {
    enki_error err =
        er_store_buf_push_u8(buf, (uint8_t)((val_q >> (k * 8u)) & 0xffu));
    if (err != ENKI_ERROR_OK) {
      return err;
    }
  }
...
static enki_error er_store_read_u64(const uint8_t* data_b, size_t data_s,
                                    size_t* off_s, uint64_t* out_q) {
...
static enki_error er_store_deserialize_nat(enki_gc* gc,
  ...
  size_t byte_s = 0;
  if (!er_store_u64_to_size(byte_q, &byte_s) || *off_s > data_s ||
      data_s - *off_s < byte_s) {
    return ENKI_ERROR_BOUNDS;
  }
  if (byte_s == 0) {
...
enki_error enki_store_read(enki_store* store, const uint8_t* key_b,
                           uint8_t* bytes, size_t cap_s, size_t* len_s) {
  if (store == NULL || store->env == NULL || key_b == NULL || len_s == NULL) {
    return ENKI_STORE_ERROR;
  }
  MDB_txn* txn;
