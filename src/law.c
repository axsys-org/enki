...
enki_value enki_law_alloc(enki_gc* gc, size_t arity_s, enki_value name_v,
                          enki_value body_v, size_t bc_len_s, size_t n_const_s,
                          uint8_t* bc_b, enki_value* const_table_v) {
  ...
  if (n_const_s > 0)
    memcpy(new->data_b, const_table_v, const_off_o);
  if (bc_len_s > 0)
    memcpy(new->data_b + const_off_o, bc_b, bc_len_s);
...
static void enki_law_compile_value(enki_interpreter* i, enki_value body_v,
                                   size_t depth_s, enki_vector* bc_b,
                                   enki_vector* const_table_v);

static void enki_law_compile_args(enki_interpreter* i, enki_app* app,
                                  size_t depth_s, enki_vector* bc_b,
                                  enki_vector* const_table_v) {
...
static void enki_law_push_const(enki_interpreter* i, enki_vector* bc_b,
                                enki_vector* const_table_v, enki_value val) {
  size_t n_const_s = enki_vector_len(const_table_v);
  enki_vector_push_copy_or_throw(i, const_table_v, &val);
  if (n_const_s > 255) {
    if (n_const_s > ENKI_BC_WIDE_MAX)
      abort();
...
static bool enki_law_is_letrec(enki_value body_v, enki_value* v,
                               enki_value* k) {
  body_v = enki_value_unind(body_v);
  if (!IS_PTR(body_v))
    return false;
...
void enki_law_compile(enki_interpreter* i, enki_value body_v, size_t arity_s,
                      enki_vector* bc_b, enki_vector* const_table_v) {
