...
static er_val eo_bad_arity(const enki_allocator* loc_a, size_t have_s,
                           size_t want_s) {
  (void)want_s;
...
static er_val eo_type(const enki_allocator* loc_a, er_val val_v) {
  ...
  case er_tag_app:
    return 3;
  default:
    return eo_tank(loc_a, val_v, "bad value tag");
  }
}
...
static er_val eo_lsh_cat(const enki_allocator* loc_a, er_val a_v,
                         size_t shift_s) {
  if (a_v == 0) {
    return 0;
  }
...
static er_val eo_app_make(const enki_allocator* loc_a, er_val fn_v,
...
er_val eo_load(const enki_allocator* loc_a, er_val idx_v, er_val width_v,
               er_val n_v) {
...
er_val eo_loadn(const enki_allocator* loc_a, er_val width_v, er_val idx_v,
                er_val n_v) {
  ENKI_PROFILE_ZONE("eo_loadn");
  return eo_load(loc_a, idx_v, width_v, n_v);
}

er_val eo_store(const enki_allocator* loc_a, er_val idx_v, er_val val_v,
                er_val width_v, er_val n_v) {
...
er_val eo_storen(const enki_allocator* loc_a, er_val width_v, er_val idx_v,
                 er_val val_v, er_val n_v) {
...
er_val eo_store8(const enki_allocator* loc_a, er_val idx_v, er_val val_v,
                 er_val n_v) {
...
er_val eo_rep(const enki_allocator* loc_a, er_val hd_v, er_val item_v,
              er_val count_v) {
...
er_val eo_row(const enki_allocator* loc_a, er_val hd_v, er_val count_v,
              er_val xs_v) {
...
er_val eo_slice(const enki_allocator* loc_a, er_val off_v, er_val count_v,
                er_val row_v) {
...
er_val eo_up(const enki_allocator* loc_a, er_val idx_v, er_val val_v,
             er_val row_v) {
...
er_val eo_law(const enki_allocator* loc_a, er_val nam_v, er_val bod_v,
              er_val ari_v) {
...
er_val eo_elim(const enki_allocator* loc_a, er_val pin_f_v, er_val law_f_v,
               er_val app_f_v, er_val zero_v, er_val nat_f_v, er_val val_v) {
...
er_val eo_exec_op66(const enki_allocator* loc_a, int op, size_t arg_s,
                    const er_val arg_v[]) {
  ENKI_PROFILE_ZONE("eo_exec_op66");
#define EO_ARITY(_want) eo_bad_arity(loc_a, arg_s, (_want))
  switch (op) {
  case OP66_TYPE:
    return arg_s == 1 ? eo_type(loc_a, arg_v[0]) : EO_ARITY(1);
  case OP66_IS_PIN:
  ...
  case OP66_SET:
    return arg_s == 2 ? eo_store(loc_a, arg_v[0], 1, 1, arg_v[1]) : EO_ARITY(2);
  case OP66_CLEAR:
    return arg_s == 2 ? eo_store(loc_a, arg_v[0], 0, 1, arg_v[1]) : EO_ARITY(2);
  case OP66_NIB:
    return arg_s == 2 ? eo_load(loc_a, arg_v[0], 4, arg_v[1]) : EO_ARITY(2);
  case OP66_LOAD:
  ...
  case EO_OP66_STORE:
    return arg_s == 4 ? eo_store(loc_a, arg_v[0], arg_v[2], arg_v[1], arg_v[3])
                      : EO_ARITY(4);
  case OP66_STORE8:
    return arg_s == 3 ? eo_store8(loc_a, arg_v[0], arg_v[1], arg_v[2])
                      : EO_ARITY(3);
  case OP66_TRUNC:
  ...
  case OP66_ROW:
    return arg_s == 3 ? eo_row(loc_a, arg_v[0], arg_v[1], arg_v[2])
                      : EO_ARITY(3);
  case OP66_REP:
    return arg_s == 3 ? eo_rep(loc_a, arg_v[0], arg_v[1], arg_v[2])
                      : EO_ARITY(3);
  case OP66_SLICE:
    return arg_s == 3 ? eo_slice(loc_a, arg_v[0], arg_v[1], arg_v[2])
                      : EO_ARITY(3);
  case OP66_WELD:
    return arg_s == 2 ? eo_weld(loc_a, arg_v[0], arg_v[1]) : EO_ARITY(2);
  case OP66_UP:
  case OP66_UP_UNIQ:
    return arg_s == 3 ? eo_up(loc_a, arg_v[0], arg_v[1], arg_v[2])
                      : EO_ARITY(3);
  case OP66_COUP:
  ...
  case OP66_SAP:
    return arg_s == 2 ? eo_thk_app(loc_a, arg_v[0], 1, arg_v + 1) : EO_ARITY(2);
  case OP66_SAP2: {
    if (arg_s != 3) {
      return EO_ARITY(3);
    }
    er_val app_v = eo_thk_app(loc_a, arg_v[0], 1, arg_v + 1);
    return er_is_good(app_v) ? eo_thk_app(loc_a, app_v, 1, arg_v + 2) : app_v;
  }
  case OP66_FORCE:
  ...
  case OP66_DEEPSEQ:
    return arg_s == 2 ? arg_v[1] : EO_ARITY(2);
  case OP66_THROW:
    return arg_s == 1 ? eo_tank(loc_a, arg_v[0], "throw") : EO_ARITY(1);
  case OP66_TRACE:
    if (arg_s != 2) {
      return EO_ARITY(2);
    }
    fprintf(stderr, "trace: %s\n", enki_pvalue(loc_a, arg_v[0]));
    return arg_v[1];
  case OP66_EQ:
...
static er_val eo_exec_op66_descriptor(const enki_allocator* loc_a, er_val tag_v,
                                      size_t arg_s, const er_val arg_v[],
                                      bool* handled_f) {
  ...
  *handled_f = true;
  ...
  case EO_S8('O', 'P', '_', 'S', 'T', 'O', 'R', 'E'):
    if (width_v == 0) {
      return arg_s == 4
                 ? eo_store(loc_a, arg_v[0], arg_v[2], arg_v[1], arg_v[3])
  ...
  case PLAN_S6('O', 'P', '_', 'M', 'E', 'T'):
    return arg_s == 1 ? eo_met(width_v, arg_v[0])
                      : eo_bad_arity(loc_a, arg_s, 1);
  default:
    break;
  }
...
static er_val eo_exec_op66_tag(const enki_allocator* loc_a, er_val tag_v,
                               size_t arg_s, const er_val arg_v[]) {
  bool handled_f = false;
  er_val out_v =
      eo_exec_op66_descriptor(loc_a, tag_v, arg_s, arg_v, &handled_f);
  if (handled_f) {
    return out_v;
  }

  int op = 0;
  return eo_op66_from_tag(tag_v, &op)
             ? eo_exec_op66(loc_a, op, arg_s, arg_v)
...
er_val eo_exec_op66_er_app(const enki_allocator* loc_a, const er_app* app) {
  ...
  if (app == NULL) {
    return eo_tank(loc_a, 0, "expected primitive row");
  }
  return eo_exec_op66_tag(loc_a, app->fn_v, app->arg_s, app->arg_v);
}
...
er_val eo_exec_op66_app(const enki_allocator* loc_a, er_val row_v) {
  ...
  if (row != NULL) {
    ...
    if (tag_v == 0 && row->arg_s > 0) {
      return eo_exec_op66_tag(loc_a, row->arg_v[0], row->arg_s - 1,
                              row->arg_v + 1);
    }
  }

  return eo_exec_op66_tag(loc_a, tag_v, arg_s, arg_v);
}

er_val eo_exec_op0(const enki_allocator* loc_a, int op, size_t arg_s,
                   const er_val arg_v[]) {
...
#define EO_ARITY(_want) eo_bad_arity(loc_a, arg_s, (_want))
  ...
  case OP0_LAW:
    return arg_s == 3 ? eo_law(loc_a, arg_v[1], arg_v[2], arg_v[0])
                      : EO_ARITY(3);
  case OP0_ELIM:
    return arg_s == 6 ? eo_elim(loc_a, arg_v[0], arg_v[1], arg_v[2], arg_v[3],
                                arg_v[4], arg_v[5])
                      : EO_ARITY(6);
  default:
    return eo_tank(loc_a, (er_val)op, "bad primitive op");
...
static er_val eo_exec_op0_tag(const enki_allocator* loc_a, er_val tag_v,
                              size_t arg_s, const er_val arg_v[]) {
  if (tag_v == 0 && arg_s > 1) {
    size_t nested_s = 0;
    if (eo_nat_to_size(arg_v[0], &nested_s) && nested_s <= OP0_ELIM) {
      return eo_exec_op0(loc_a, (int)nested_s, arg_s - 1, arg_v + 1);
    }
    return eo_tank(loc_a, arg_v[0], "bad primitive tag");
  }
...
er_val eo_exec_op0_er_app(const enki_allocator* loc_a, const er_app* app) {
  ...
  return eo_exec_op0_tag(loc_a, app->fn_v, app->arg_s, app->arg_v);
}
...
er_val eo_exec_op0_app(const enki_allocator* loc_a, er_val row_v) {
  ...
  return eo_exec_op0_er_app(loc_a, row);
}
