...
static void enki_print_value_sb(const enki_allocator* cat_a,
                                enki_string_builder* sb, er_val val_v);
...
static void enki_print_direct_text_raw(enki_string_builder* sb,
                                       uint64_t dir_q) {
  while (dir_q > 0) {
    enki_sb_append_char(sb, (char)(dir_q & 0xffu));
    dir_q >>= 8u;
...
static void enki_print_bat(const enki_allocator* cat_a, enki_string_builder* sb,
                           const er_bat* bat) {
...
static bool enki_print_law_name_raw(const enki_allocator* cat_a,
                                    enki_string_builder* sb, er_val name_v) {
  if (er_is_cat(name_v)) {
...
static void enki_print_law(const enki_allocator* cat_a, enki_string_builder* sb,
                           const er_law* law) {
  enki_sb_append_lit(sb, "{");
...
static void enki_print_app(const enki_allocator* cat_a, enki_string_builder* sb,
                           const er_app* app) {
  enki_sb_append_lit(sb, "(");
...
char* enki_print_value(const enki_allocator* cat_a, er_val val_v,
                       size_t* out_s) {
  size_t def_s = 0;
  if (out_s == NULL) {
    out_s = &def_s;
...
char* enki_log_value(const enki_allocator* cat_a, er_val val_v, size_t* out_s) {
  UNUSED(cat_a);
  UNUSED(val_v);
  UNUSED(out_s);
  return "";
