...
#endif
#ifndef PLAN_S4
#define PLAN_S4(a, b, c, d)                                                    \
  ((enki_value)(PLAN_S3(a, b, c) | (PLAN_CH(d) << 24u)))
#endif
#ifndef PLAN_S5
#define PLAN_S5(a, b, c, d, e)                                                 \
  ((enki_value)(PLAN_S4(a, b, c, d) | (PLAN_CH(e) << 32u)))
#endif
#ifndef PLAN_S6
#define PLAN_S6(a, b, c, d, e, f)                                              \
  ((enki_value)(PLAN_S5(a, b, c, d, e) | (PLAN_CH(f) << 40u)))
#endif
#ifndef PLAN_S7
#define PLAN_S7(a, b, c, d, e, f, g)                                           \
  ((enki_value)(PLAN_S6(a, b, c, d, e, f) | (PLAN_CH(g) << 48u)))
#endif
...
static enki_error plan_eval_whnf_inner(enki_plan* plan, enki_value val_v,
                                       enki_value* out_v);
static enki_error plan_eval_nf_inner(enki_plan* plan, enki_value val_v,
                                     enki_value* out_v);
static enki_error plan_apply_inner(enki_plan* plan, enki_value fn_v,
                                   size_t n_args_s, const enki_value* args_v,
                                   enki_value* out_v);
static enki_error plan_arity_value(enki_plan* plan, enki_value val_v,
                                   size_t* out_s);

static enki_error plan_fail(enki_plan* plan, enki_error error,
                            enki_value val_v) {
  if (plan != NULL) {
    plan->error = error;
    plan->error_v = val_v;
...
static enki_error plan_nat_inner(enki_plan* plan, enki_value val_v,
                                 enki_value* out_v) {
...
static enki_error plan_alloc_app(enki_plan* plan, enki_value fn_v,
                                 size_t n_args_s, const enki_value* args_v,
                                 enki_value* out_v) {
  if (n_args_s == 0) {
    *out_v = fn_v;
...
static enki_error plan_make_app(enki_plan* plan, enki_value fn_v,
                                size_t n_args_s, const enki_value* args_v,
                                enki_value* out_v) {
  if (n_args_s == 0) {
    *out_v = fn_v;
...
static void plan_spine_fill(enki_value val_v, enki_value* args_v,
                            size_t* cursor_s) {
  if (!plan_is_kind(val_v, ENKI_APP))
    return;
  enki_app* app = ENKI_AS(enki_app, val_v);
...
static enki_error plan_spine_open(enki_plan* plan, enki_value val_v,
                                  enki_plan_spine* out) {
...
static enki_error plan_row_open(enki_plan* plan, enki_value val_v,
                                enki_plan_row* out) {
...
static enki_error plan_require_argc(enki_plan* plan, size_t have_s,
                                    size_t want_s) {
  if (have_s != want_s)
...
static enki_error plan_arg_nat(enki_plan* plan, const enki_value* args_v,
                               size_t idx_s, enki_value* out_v) {
  return plan_nat_inner(plan, args_v[idx_s], out_v);
}

static enki_error plan_arg_small(enki_plan* plan, const enki_value* args_v,
                                 size_t idx_s, size_t* out_s) {
  enki_value nat_v = 0;
...
static enki_error plan_structural_equal(enki_plan* plan, enki_value a_v,
                                        enki_value b_v, enki_value* out_v) {
  ...
  case ENKI_PIN: {
    ...
    for (size_t k = 0; k < a->n_subpins_s; k++) {
      err =
          plan_structural_equal(plan, a->subpins_v[k], b->subpins_v[k], out_v);
      if (err != ENKI_ERROR_OK || *out_v == 0)
        return err;
    }
    *out_v = 1;
...
static enki_error plan_case_n(enki_plan* plan, const enki_value* args_v,
                              size_t n_args_s, size_t n_cases_s,
                              enki_value* out_v) {
...
static enki_error plan_dispatch_op66(enki_plan* plan, enki_value row_v,
                                     enki_value* out_v) {
  enki_plan_row row;
  ...
  case OP66_UNPIN:
    if ((err = plan_require_argc(plan, row.n_args_s, 1)) != ENKI_ERROR_OK)
      break;
    *out_v = plan_is_kind(row.args_v[0], ENKI_PIN)
                 ? ENKI_AS(enki_pin, row.args_v[0])->inner_v
                 : 0;
    break;
  case OP66_ARITY:
    if ((err = plan_require_argc(plan, row.n_args_s, 1)) != ENKI_ERROR_OK)
  ...
  case OP66_NAME:
    if ((err = plan_require_argc(plan, row.n_args_s, 1)) != ENKI_ERROR_OK)
      break;
    *out_v = plan_is_kind(row.args_v[0], ENKI_LAW)
                 ? ENKI_AS(enki_law, row.args_v[0])->name_v
                 : 0;
    break;
  case OP66_BODY:
    if ((err = plan_require_argc(plan, row.n_args_s, 1)) != ENKI_ERROR_OK)
      break;
    *out_v = plan_is_kind(row.args_v[0], ENKI_LAW)
                 ? ENKI_AS(enki_law, row.args_v[0])->body_v
                 : 0;
    break;
  case OP66_HD:
    if ((err = plan_require_argc(plan, row.n_args_s, 1)) != ENKI_ERROR_OK)
    ...
    *out_v = plan_is_kind(row.args_v[0], ENKI_APP)
                 ? ENKI_AS(enki_app, row.args_v[0])->fn_v
                 : row.args_v[0];
    break;
  case OP66_LAST:
    if ((err = plan_require_argc(plan, row.n_args_s, 1)) != ENKI_ERROR_OK)
      ...
      *out_v = 0;
      ...
      if (app->n_args_s <= 1) {
        *out_v = app->fn_v;
        ...
        err = plan_alloc_app(plan, app->fn_v, app->n_args_s - 1, app->args_v,
                             out_v);
      }
    }
    break;
  ...
  case OP66_WELD:
    if ((err = plan_require_argc(plan, row.n_args_s, 2)) != ENKI_ERROR_OK)
      break;
    {
      enki_app* x = plan_is_kind(row.args_v[0], ENKI_APP)
                        ? ENKI_AS(enki_app, row.args_v[0])
                        : NULL;
      enki_app* y = plan_is_kind(row.args_v[1], ENKI_APP)
                        ? ENKI_AS(enki_app, row.args_v[1])
                        : NULL;
  ...
  case OP66_COUP:
    ...
    if (!plan_is_kind(row.args_v[1], ENKI_APP)) {
      *out_v = row.args_v[0];
      ...
      err = plan_apply_inner(plan, row.args_v[0], app->n_args_s, app->args_v,
                             out_v);
    }
    break;
  case OP66_SZ:
  ...
  case OP66_CASE:
    if ((err = plan_require_argc(plan, row.n_args_s, 3)) != ENKI_ERROR_OK)
      break;
    if (!plan_small_nat(row.args_v[0], &n_s) ||
        !plan_is_kind(row.args_v[1], ENKI_APP)) {
      *out_v = row.args_v[2];
    } else {
      enki_app* app = ENKI_AS(enki_app, row.args_v[1]);
  ...
  case OP66_CASE15:
  case OP66_CASE16:
    err = plan_case_n(plan, row.args_v, row.n_args_s,
                      (size_t)(op - OP66_CASE2 + 2), out_v);
    break;
  case OP66_NIL:
    if ((err = plan_require_argc(plan, row.n_args_s, 1)) != ENKI_ERROR_OK)
      break;
  ...
  case OP66_IFZ:
    if ((err = plan_require_argc(plan, row.n_args_s, 3)) != ENKI_ERROR_OK)
      break;
    if ((err = plan_eval_whnf_inner(plan, row.args_v[0], &a_v)) !=
        ENKI_ERROR_OK)
      break;
    *out_v =
        (op == OP66_IF ? a_v != 0 : a_v == 0) ? row.args_v[1] : row.args_v[2];
    break;
  case OP66_SEQ:
    if ((err = plan_require_argc(plan, row.n_args_s, 2)) != ENKI_ERROR_OK)
      break;
  ...
  case OP66_SEQ2:
    ...
    if ((err = plan_eval_whnf_inner(plan, row.args_v[0], &a_v)) !=
        ENKI_ERROR_OK)
      break;
    if ((err = plan_eval_whnf_inner(plan, row.args_v[1], &a_v)) !=
        ENKI_ERROR_OK)
      break;
    *out_v = row.args_v[2];
    break;
  case OP66_SEQ3:
    ...
    if ((err = plan_eval_whnf_inner(plan, row.args_v[1], &a_v)) !=
        ENKI_ERROR_OK)
      break;
    if ((err = plan_eval_whnf_inner(plan, row.args_v[2], &a_v)) !=
        ENKI_ERROR_OK)
      break;
    *out_v = row.args_v[3];
    break;
  ...
  case OP66_SAP2:
    ...
    if ((err = plan_eval_whnf_inner(plan, row.args_v[1], &a_v)) !=
        ENKI_ERROR_OK)
      break;
    if ((err = plan_eval_whnf_inner(plan, row.args_v[2], &b_v)) !=
        ENKI_ERROR_OK)
      break;
    if ((err = plan_apply_inner(plan, row.args_v[0], 1, &a_v, &c_v)) !=
        ENKI_ERROR_OK)
      break;
...
static enki_error plan_elim(enki_plan* plan, enki_value p_v, enki_value l_v,
                            enki_value a_v, enki_value z_v, enki_value m_v,
                            enki_value o_v, enki_value* out_v) {
...
static enki_error plan_dispatch_op0(enki_plan* plan, enki_value row_v,
                                    enki_value* out_v) {
  enki_plan_row row;
  enki_error err = plan_row_open(plan, row_v, &row);
  if (err != ENKI_ERROR_OK)
  ...
  case OP0_PIN: {
    ...
    if ((err = plan_eval_whnf_inner(plan, row.args_v[0], &inner_v)) !=
        ENKI_ERROR_OK)
      break;
    uint8_t hash_b[32] = {0};
  ...
  case OP0_LAW: {
    ...
    if ((err = plan_arg_small(plan, row.args_v, 0, &arity_s)) != ENKI_ERROR_OK)
      break;
    *out_v = enki_law_alloc(plan->gc, arity_s + 1, row.args_v[1], row.args_v[2],
                            0, 0, NULL, NULL);
    break;
  }
  case OP0_ELIM:
    if ((err = plan_require_argc(plan, row.n_args_s, 6)) != ENKI_ERROR_OK)
      break;
    err = plan_elim(plan, row.args_v[0], row.args_v[1], row.args_v[2],
                    row.args_v[3], row.args_v[4], row.args_v[5], out_v);
    break;
  default:
    err = plan_fail(plan, ENKI_ERROR_BAD_TAG, row.tag_v);
    break;
...
static enki_error plan_dispatch_group(enki_plan* plan, enki_value group_v,
                                      enki_value row_v, enki_value* out_v) {
  size_t group_s = 0;
...
static enki_error plan_run_body(enki_plan* plan, size_t depth_s,
                                const enki_value* env_v, enki_value body_v,
                                enki_value* out_v) {
  ...
  if (app->fn_v == 0) {
    ...
    if (app->n_args_s == 2) {
      ...
      enki_value arg_v = 0;
      enki_error err =
          plan_run_body(plan, depth_s, env_v, app->args_v[0], &fn_v);
      if (err != ENKI_ERROR_OK)
        return err;
  ...
  if (app->fn_v == 1 && app->n_args_s == 2) {
    ...
    enki_error err =
        plan_run_body(plan, depth_s, env_v, app->args_v[0], &bound_v);
    if (err != ENKI_ERROR_OK)
      return err;
...
static enki_error plan_apply_law(enki_plan* plan, enki_value self_v,
                                 enki_law* law, size_t n_args_s,
...
static enki_error plan_reduce_exact(enki_plan* plan, enki_value fn_v,
                                    size_t n_args_s, const enki_value* args_v,
                                    enki_value* out_v) {
  plan->apply_s++;
  if (plan_is_kind(fn_v, ENKI_LAW)) {
    return plan_apply_law(plan, fn_v, ENKI_AS(enki_law, fn_v), n_args_s, args_v,
                          out_v);
  }
  if (plan_is_kind(fn_v, ENKI_PIN)) {
    enki_pin* pin = ENKI_AS(enki_pin, fn_v);
    if (plan_is_kind(pin->inner_v, ENKI_LAW)) {
      return plan_apply_law(plan, fn_v, ENKI_AS(enki_law, pin->inner_v),
                            n_args_s, args_v, out_v);
    }
  ...
  for (;;) {
    ...
    case ENKI_APP: {
      ...
      if (arity_s == 0 || app->n_args_s < arity_s) {
        if (fn_v != app->fn_v ||
            (arity_s == 0 && plan_is_kind(fn_v, ENKI_APP))) {
