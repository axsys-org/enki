...
void op0_pin(enki_interpreter* i) {
  ...
  uint8_t hash_b[32] = {0};
  enki_value pin =
      enki_pin_alloc(i->gc, hash_b, inner_v, n_subpins_s, subpins_v);
...
void op0_law(enki_interpreter* i) {
  ...
  enki_value law = enki_law_alloc(i->gc, (size_t)arity_s, name_v, body_v,
                                  bc_len_s, n_const_s, bc_b, const_table_v);
  i->sp -= 2;
