...
`;
  ...
  const rootLines = (await readFile(join(work, "snap", "root.plan"), "utf8"))
    .trim()
    .split("\n");
  ...
  while (pending.length > 0) {
    ...
    const text = await readFile(join(work, "snap", `${hash}.plan`), "utf8");
  ...
  if (!rootText.includes('#law "main"'))
    throw new Error("saved JPLAN root is not the Reaver main function");
