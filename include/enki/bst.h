...
typedef enki_bst_value (*enki_bst_pack_fn)(enki_bst_key key, int macro,
                                           enki_bst_value value,
                                           enki_bst_value left,
                                           enki_bst_value right, void* user);
...
ENKI_BST_API enki_bst_node* enki_bst_new_node(enki_allocator allocator,
                                              enki_bst_key key,
...
ENKI_BST_API enki_bst_node* enki_bst_find(enki_bst_node* root,
                                          enki_bst_key key);
ENKI_BST_API int enki_bst_get(enki_bst_node* root, enki_bst_key key,
                              int* out_macro, enki_bst_value* out_value,
                              enki_bst_node** out_node);

/* destructive put: inserts or updates, then returns the touched node, or NULL
 * on OOM */
ENKI_BST_API enki_bst_node* enki_bst_put(enki_allocator allocator,
                                         enki_bst_node** root, enki_bst_key key,
                                         enki_bst_value value, int macro);
ENKI_BST_API enki_bst_node* enki_bst_tree_put(enki_bst_tree* t,
                                              enki_bst_key key,
                                              enki_bst_value value, int macro);
...
ENKI_BST_API enki_bst_node*
enki_bst_put_persistent(enki_allocator allocator, enki_bst_node* root,
                        enki_bst_key key, enki_bst_value value, int macro);

/* inorder traversal: left, node, right; returns non-zero if stopped early */
ENKI_BST_API int enki_bst_walk(enki_bst_node* root, enki_bst_walk_fn fn,
                               void* user);
ENKI_BST_API size_t enki_bst_count(enki_bst_node* root);

/* allocates an inorder array of node pointers; caller releases with
 * enki_bst_free_array */
ENKI_BST_API enki_bst_node** enki_bst_walk_array(enki_allocator allocator,
                                                 enki_bst_node* root,
                                                 size_t* out_count);
ENKI_BST_API enki_bst_node** enki_bst_tree_walk_array(const enki_bst_tree* t,
                                                      size_t* out_count);
...
*/
ENKI_BST_API enki_bst_value enki_bst_fold_adt(enki_bst_node* root,
                                              enki_bst_value empty,
                                              enki_bst_pack_fn pack,
                                              void* user);
...
static enki_bst_node* enki_bst__put_copy(enki_allocator allocator,
                                         enki_bst_node* root, enki_bst_key key,
                                         enki_bst_value value, int macro,
                                         int* ok) {
  enki_bst_node* n;
  ...
  if (ENKI_BST_KEY_LESS(key, root->key)) {
    enki_bst_node* l =
        enki_bst__put_copy(allocator, root->left, key, value, macro, ok);
    if (!*ok) {
    ...
    enki_bst_node* r =
        enki_bst__put_copy(allocator, root->right, key, value, macro, ok);
    if (!*ok) {
      allocator.free(allocator.ctx, n);
      return 0;
    }
