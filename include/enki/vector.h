 ...
 */
enki_vector* enki_vector_create_sized(const enki_allocator* allocator_a,
                                      size_t elem_size_s);

/**
 * VM-facing vector helpers that throw through the interpreter on failure.
 */
enki_vector* enki_vector_create_sized_or_throw(
    enki_interpreter* i, const enki_allocator* allocator_a, size_t elem_size_s);
void enki_vector_push_u8_or_throw(enki_interpreter* i, enki_vector* vector,
                                  uint8_t item_v);
void enki_vector_push_copy_or_throw(enki_interpreter* i, enki_vector* vector,
                                    const void* item_v);
 ...
 * item_v value_v, so callers that store NULL should check enki_vector_len
 * first.
 */
 ...
 */
enki_status enki_vector_set_copy(enki_vector* vector, size_t index_i,
                                 const void* item_v);
