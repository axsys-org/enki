/*
    ...
    notes:
        ...
        - materialized output is always NUL-terminated, but may contain embedded NULs.
        - do not memcpy/copy an initialized builder by value; it has internal storage.
*/
...
/* lifecycle */
ENKI_SB_API void enki_sb_init(enki_string_builder* sb, const enki_allocator* allocator);
...
ENKI_SB_API int enki_sb_reserve_pieces(enki_string_builder* sb, size_t additional_piece_count);

/* appending */
ENKI_SB_API int enki_sb_append_ref(enki_string_builder* sb, const char* data, size_t len);
...
ENKI_SB_API int   enki_sb_write(const enki_string_builder* sb,
...
static char* enki_sb__emit_u64(char* dst, uint64_t value,
                               unsigned base, int uppercase)
{
    char tmp[64];
    size_t n = 0;
                              ...
                              char* dst, size_t dst_size, size_t* out_len)
{
    ...
    for (i = 0; i < sb->count; ++i) {
        ...
        switch ((enki_sb_piece_kind)piece->kind) {
            ...
            case ENKI_SB_PIECE_U64:
                p = enki_sb__emit_u64(p, piece->u.u64,
                                       piece->base,
                                       piece->uppercase);
                break;
