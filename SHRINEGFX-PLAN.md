...
> **Status: ✅ DONE (2026-07-10)** — `foil/shrine/kernel-ir.plan`: NE.8d.1's atom set
> as 5-row steps (mov/load/store/add/mul/neg/cge/sel/retif) with nat-packed operand
> refs (const/register/tid) and per-buffer thread strides; scalars zigzag-encoded
> signed integers (test vectors f32-exact by construction — the audit tier needs no
> soft-float); the PSA sandwich **generated by law** (7,008 steps from the blade
> arithmetic, antireverse folded into phase-B signs) and **interpreted by law** in
> 1.3s for two threads.  Fixture `kernel_ir_interpreter_matches_c_reference`
> (deviceless): the wisp-run checksums (302828, 486636) match an independent C
> table-walk EXACTLY.  25/25 fixtures.
